...
```{r, message=FALSE, warning=FALSE}
# Load gene data
# WILL BE CHANGED TO IMPORT FROM GEO
cop1_eset <- readRDS(
...
`min_mean_cpm` = 150 within either of the two groups of interest,
*wild type* or *knock-out*. We set a very large threshold of 150
...
```{r}
# Fine genes with minimum mean count of 150 at least in one of the two groups
min_mean_cpm <- 150
...
Here we will generate a heatmap of top 50 most variable genes.
...
plot_heatmap(cop1_timeseq, num.feat = 50)
```

In the heatmap the colums are ordered by experimental group, replicate 
...
the red-and-blue palette corresponding to high-and-low respectively. Even the
first look at the data, can show us patterns present in the data -- within each 
...
```{r pca-samples-group, fig.wide = TRUE, fig.height=4, fig.width=8}
plot_sample_pca(cop1_timeseq, col.var = "group", size = 5) 
```

```{r pca-samples-time, fig.wide = TRUE, fig.height=4, fig.width=8}
...
```{r pca-genes, fig.width = 8, fig.height = 7}
plot_ts_pca(
  cop1_timeseq, 
...
Here we pick 600 genes for clustering, whcih is roughly half of the
number of genes in the object.

```{r}
params_for_clustering <- list(
  dynamic = TRUE, 
  dynamic_cut_params = list(deepSplit = TRUE, minModuleSize = 50))
  
cop1_timeseq <- cluster_timecourse_features(
  cop1_timeseq, n_top_feat = 600, groups = "all",
...
```{r gene-clusters, fig.width = 7, fig.height = 5, fig.wide = TRUE}
...
to their pattern of response to LPS treatment. We see cluster  C6
generally inhibited. Genes in C1 spike right after LPS was applied. C4
...
```{r tmp-de-intersect, fig.width=8, fig.height=4}
library(UpSetR)
...
Here we print out 5 of the genes with highest $R^2$ value and 
...
We only print 5 genes due to file size limits (you should inspect more 
DE genes).
...
```{r genes-de-trajectory, fig.width = 7, fig.height = 2.5, fig.wide = TRUE}
...
Below we print we print for example a plot for cluster C1.

```{r go-enrich-clst,  fig.width = 8, fig.height = 4, fig.wide = TRUE}
clst <- "C1"
plot_enrichment(
