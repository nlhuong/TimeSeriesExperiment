...
check_vistimeseq <- function(object){
    ...
    if (nrow_feature_data > 0) {
        if (nrow_raw != nrow_feature_data) {
            msg <- paste0(
              "Number of rows in feature.data is ", nrow_feature_data,
              ". Should be equal to number of rows in raw.data, ", nrow_raw)
            errors <- c(errors, msg)
        }
        if (!all(object@feature.data$feature == rownames(object@raw.data))) {
            msg <- paste("3. Inconsistent feature.data features and rownames",
                          "in raw.data")
            errors <- c(errors, msg)
        }
        if (all(!is.null(object@data),
                !all(object@feature.data$feature == rownames(object@data)))) {
            msg <- paste("4. Inconsistent feature.data features and rownames",
                          "in raw.data")
            errors <- c(errors, msg)
        }
    }
    if (!is.null(object@data.collapsed)) {
        if (!all(object@sample.data.collapsed$sample ==
            ...
            msg <- paste("5. Inconsistent row names in sample.data.collapsed",
...
impute_data <- function(raw.data, sample.data) {
    ...
    group <- time <- category <- NULL
    if(!any(c("group", "replicate", "time") %in% colnames(sample.data))) {
        stop("Missing columns 'group', 'replicate', or 'time' in 'sample.data'")
    }
    ...
    for(id in unique(sample.data$category)) {
        smp_gr_time <- filter(sample.data, category == id)
        ...
        if(any(missing_time)) {
            ...
            for (i in which(missing_time)) {
                ...
                surrounding_samples <- smp_gr_time %>% 
                  filter(time %in% t_surround)
...
#'
#' @description The \code{vistimeseq} object is the main object in the time
...
#' membership and time.
#' @slot feature.data A \code{data.frame} object were rows are features 
...
#' @slot group A character vector of length \code{ncol(raw.data)} 
...
#' @slot dim.red List of stored dimmensional reductions; 
#' named by technique used.
#' @slot cluster.features A list storing results from timecourse (feature)
#' clustering.
#' @slot diff.expr A list storing results of differential 
#' expression analysis.
#'
#' @name vistimeseq
#' @rdname vistimeseq
...
vistimeseq <- function(
    ...
    if(!is.null(time_column)){
        if (!time_column %in% colnames(sample.data)){
          stop("No 'time_column', ", time_column, " in sample.data")
        }
    ...
    if (!is.null(replicate_column)) {
        if (! replicate_column %in% colnames(sample.data)) {
          stop("No 'replicate_column', ", replicate_column, " in sample.data")
        }
    ...
    if(is.null(feature.data)) {
        ...
        if(!all(rownames(raw.data) %in% rownames(feature.data))){
            stop("Not all rownames(raw.data) in rownames(feature.data)")
        }
        feature.data <- feature.data[rownames(raw.data), , drop = FALSE]
...
#' and updates the colums.
#' 
#' @param sample.data A \code{data.frame} object were rows are samples
#' (observations) and columns are sample attributes 
#' (e.g. group/condition replicate, time)
#' @param time_column A character string equal to one of the column names
#' of \code{sample.data}. This is an alternative, way of specifying the time
...
#'
#' @description Initializes the vistimeseq object from SummarizedExperiment
#' and populates the time, replicate, and group slots.
#'
#' @param sumexp SummarizedExperiment object
#' @param project Project name (string)
#' @param time_column A character string equal to one of the column names
#' of \code{sample.data}. This is an alternative, way of specifying the time
#' corresponding to samples, if \code{time} argument is not provided.
...
#'    feature = paste0("F", 1:100)
#')
#'test_sumexp <- SummarizedExperiment(
#'    assays = list(counts = raw),
#'    rowData = feature.data, colData = pheno.data)
#'    
#'test_vistimeseq <- vistimeseq_from_SummarizedExperiment(
#'    test_sumexp, time_column = "time", group_column = "group",
#'    replicate_column = "replicate")
#' test_vistimeseq
vistimeseq_from_SummarizedExperiment <- function(
  sumexp,
  time_column,
  replicate_column = NULL,
  group_column = NULL,
  ...
  feature.data <- as.data.frame(rowData(sumexp))
  sample.data <- as.data.frame(colData(sumexp))
  sample.data <- update_info(sample.data, time_column, replicate_column,
                             group_column)
  raw.data <- data.frame(assays(sumexp)[[1]], check.names = FALSE)
  object <- vistimeseq(
    project = project,
    raw.data = raw.data,
    feature.data = feature.data,
