...
opts_chunk$set(
  ...
  warning = FALSE, fig.width = 8, fig.height = 7) 
```
...
```{r}
# Fine genes with minimum mean count of 100 at least in one of the two groups
min_mean_cpm <- 50
...
params_for_clustering <- list(
  dynamic = TRUE, 
  dynamic_cut_params = list(deepSplit = TRUE, minModuleSize = 60))
  
cop1_timeseq <- cluster_timecourse_features(
  cop1_timeseq, n_top_feat = 1000, groups = "all",
...
to their pattern of response to LPS treatment. We see cluster C2, C5, and C6
generally inhibited. Genes in C3 spike right after LPS was applied. C8
shows moderate secondary response, and C1 exhibit late increase in expression.
...
Below we print we print for example a plot for cluster C6.
