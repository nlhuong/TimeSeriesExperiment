...
cnts <- read_csv(
  bfcrpath(BiocFileCache(ask = FALSE), rnames = urls[1])) %>%
  column_to_rownames("X1") %>%
  as.matrix()
gene.data <- read_csv(
  bfcrpath(BiocFileCache(ask = FALSE), rnames = urls[2])) %>%
  as.data.frame() %>%
  column_to_rownames("X1")
pheno.data <- read_csv(
  bfcrpath(BiocFileCache(ask = FALSE), rnames = urls[3])) %>%
  as.data.frame() %>%
  column_to_rownames("X1")
```
...
Alternatively, we can build `vistimeseq` object from a `ExpressionSet`
...
cat("Out of all", n_features(cop1_timeseq), ", there were",
    length(de_any_tmp), "were found differentially expressed ",
    "at any timepoint.")
```
