...
`TimeSeriesExperiment` is a comprehensive time course
...
the time variablie MUST be numeric.
...
object using `makeTimeSeriesExperimentFromExpressionSet()` funtion, and 
...
to obtain their mean behaviour. To do this we can use `collapseReplicates()`
...
This data wranggling step can be performed with `makeTimeSeries()`
...
`addLags()` function, whcih appends lag features to "tc" and "tc_collapsed"
...
`TimeSeriesExperiment` provides a wrapper `timepointDE()` for differental 
