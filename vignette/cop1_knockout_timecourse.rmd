...
```{r pca-genes, fig.width = 9, fig.height = 7}
plot_ts_pca(
  cop1_timeseq, 
...
cat("Out of all", n_features(object), ", there were", 
...
```{r tmp-de-intersect, fig.width=9, fig.height=5}
library(UpSetR)
...
saveRDS(cop1_timeseq@diff.expr$trajectory_de, 
        "../inst/extdata/cop1_cmp_sum_scale_trajectory_de.rds")
```

```{r, echo=FALSE}
cop1_timeseq@diff.expr$trajectory_de <- readRDS(
  system.file("extdata", "cop1_cmp_sum_scale_trajectory_de.rds",
              package = "vistimeseq", mustWork = TRUE))
```
...
variance explained by groups). There are `r sum(genes_de_trajectory$R2 > 0.7)`
genes with $R^2 \ge 0.7$ which constitutes a fraction of 
`r sum(genes_de_trajectory$R2 > 0.7)/nrow(object@data)` 
...
```{r genes-de-trajectory, fig.width = 14, fig.height = 14, fig.wide = TRUE}
...
metric. Additionally, if you are interested in finding out genes DE in specific time intervals e.g. beginning or end of an experiment, you can choose to keep 
...
<!---> 
DE gene profile clusters {#fun-clusters .unnumbered}
-------------------------------------------------
Over Representation Analysis {#ora .unnumbered}
-------------------------------------------------
edge,
maSigPro,
timecourse,
...
using `pathway_enrichment()` which is built on top of `goanna()` and `kegga()` functions from `limma` package. The `features` argument provided 
...
saveRDS(enrich_res, "../inst/extdata/go_enrich_res.rds")
```

```{r, echo=FALSE, eval = TRUE}
enrich_res <- readRDS(
  system.file("extdata", "go_enrich_res.rds", package = "vistimeseq", 
              mustWork = TRUE))
```
